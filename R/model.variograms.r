model.variograms <- function(sbt.type, data, model='Exp')
{
  sbt.vgm <- NULL; vgm.ver <- NULL; vgm.hor <- NULL; expvar.hor <- NULL; expvar.ver <- NULL
  sbt.max <- max(data[,sbt.type])
  for(sbt.class in 1:sbt.max)
  {
    ### Fit vertical one with different models
    sbt.indicator <- as.numeric(data[,sbt.type]==sbt.class)
    gstatobject <- gstat(id='sbt',formula= sbt.indicator ~ 1, locations= ~ X+Y+geoz, data=data)
